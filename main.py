...
def sample_top_p(probs: torch.Tensor, p: float):
    ...
    next_token = torch.multinomial(probs_sort, num_samples=1) # 返回sample到的index
...
def generate(prompts: List[str], model: Transformer, tokenizer: Tokenizer, *, max_tokens: int,  temperature: float, chunk_size: int = None):
    ...
    for s in range(0, max_prompt_len, chunk_size):
        ...
        print(f'prelogits:{prelogits.shape}')
        ...
        print(f'last_token_prelogits:{last_token_prelogits.shape}')
    ...
    if generated_tokens:
        ...
        for i, x in enumerate(encoded_prompts):
            # print(f'generated_tokens:{generated_tokens[i]}')
