...
def test(model_path: str):
    ...
    tokenizer = Tokenizer(str(Path(model_path) / "tokenizer.model"))
    pmpts = ['I am a freshman of westlake university']
    encoded_prompts = [tokenizer.encode(prompt, bos=True) for prompt in pmpts]
    
    print(f'encoded_prompts:{encoded_prompts}')
    loaded = torch.load(str(Path(model_path) / "consolidated.00.pth"), mmap=True)
    # embedding_params = loaded['model']['encoder.embed_tokens.weight']
    embed_list = []
    for k, v in loaded.items():
        if k.startswith("tok_embeddings"):
            embed_list.append(v)
    # print(f'embed_list:{embed_list[0].shape}')
    tok_embed = embed_list[0]
    # [token1, token2, token3, ...] --> [embedding1, embedding2, embedding3, ...]
    # [int, int, int, ...] --> [tensor, tensor, tensor, ...]

    # embedding_layer = nn.Embedding(len(tok_embed),4096)
    # h = embedding_layer.from_pretrained(tok_embed)
    embedding_layer = nn.Embedding.from_pretrained(tok_embed)
    # print(f'encoded embeddings:{h.shape}')
    IF = neuron.IFNode(v_reset=None, v_threshold=1.0, step_mode='s')
