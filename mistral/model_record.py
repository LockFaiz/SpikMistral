...
class Attention(nn.Module):
    def __init__(self, args: ModelArgs):
        ...
        self.fr_wq_IF = monitor.OutputMonitor(self.wq_IF, neuron.IFNode, cal_firing_rate)
        self.wk_IF = neuron.IFNode(step_mode='m')
        self.fr_wk_IF = monitor.OutputMonitor(self.wk_IF, neuron.IFNode, cal_firing_rate)
        self.wv_IF = neuron.IFNode(step_mode='m')
        self.fr_wv_IF = monitor.OutputMonitor(self.wv_IF, neuron.IFNode, cal_firing_rate)
        # self.wo_IF = neuron.IFNode(step_mode='m')
        # self.fr_wo_IF = monitor.OutputMonitor(self.wo_IF, neuron.IFNode, cal_firing_rate)
    ...
    def forward(
        ...
        xk = xk_pos_ - xk_neg_
        xv = xv_pos_ - xv_neg_
        assert len(self.fr_wq_IF.records) == 2, 'Wq fr has records number != 2'
        assert len(self.fr_wk_IF.records) == 2, 'Wk fr has records number != 2'
        assert len(self.fr_wv_IF.records) == 2, 'Wv fr has records number != 2'
        print(f'posive attention Wq fr: {self.fr_wq_IF.records[0]}; negative attention Wq fr: {self.fr_wq_IF.records[1]}')
        print(f'posive attention Wk fr: {self.fr_wk_IF.records[0]}; negative attention Wk fr: {self.fr_wk_IF.records[1]}')
        print(f'posive attention Wv fr: {self.fr_wv_IF.records[0]}; negative attention Wv fr: {self.fr_wv_IF.records[1]}')
...
class FeedForward(nn.Module):
    def __init__(self, args: ModelArgs):
        ...
        self.fr_FeedW1_IF = monitor.OutputMonitor(self.w1_IF, neuron.IFNode, cal_firing_rate)
        # self.w2_IF = neuron.IFNode(step_mode='m')
        # self.fr_FeedW2_IF = monitor.OutputMonitor(self.w2_IF, neuron.IFNode, cal_firing_rate)
        self.w3_IF = neuron.IFNode(step_mode='m')
        self.fr_FeedW3_IF = monitor.OutputMonitor(self.w3_IF, neuron.IFNode, cal_firing_rate)
    ...
    def forward(self, x) -> torch.Tensor:
        ...
        assert len(self.fr_FeedW3_IF.records) == 2, 'Feedforward W3 fr has records number != 2' 
        assert len(self.fr_FeedW1_IF.records) == 1, 'Feedforward W1 fr has records number != 1' 
        print(f'posive Feedforward W3 fr: {self.fr_FeedW3_IF.records[0]}; negative Feedforward W3 fr: {self.fr_FeedW3_IF.records[1]}')
        print(f'posive Feedforward W1 fr: {self.fr_FeedW1_IF.records[0]}')
        # 计算w1 * w3
    ...
    def forward_partial(
        ...
        for local_layer_id, layer in enumerate(self.layers.values()):
            ...
            if cache is not None:
                ...
                cache_view = None
            print(f'Current layer index:{local_layer_id}')
